...
\begin{figure}[ht]
    \centering
    \incfig[.6]{problema}
    \caption{Problema ai valori iniziali}\label{fig:problema}
\end{figure}
...
\begin{proof}
    ...
    funzione continua con graico in \(D\), allora \(x\) è soluzione di \((P)\)
    se e solo se 
    \[
...
\begin{example}
    ...
    \begin{figure}[ht]
        ...
        \begin{tikzpicture}
            ...
            \addplot[domain=-10:10, samples=100]{sqrt(abs(x))};
            \end{axis}
        \end{tikzpicture}
...
\begin{figure}[ht]
    ...
    \caption{pennelloPeano}
    \label{fig:pennellopeano}
\end{figure}
\end{example}
...
abbiamo che è definit in \([-\alpha, \alpha]\) con \(\alpha = \min(a, b/M)\) e
\(M = \max |f| = \max_{R_{a,b} }  |x^2| = \max_{x \in [1-b, 1+b]}{(1+b)}^2\).
