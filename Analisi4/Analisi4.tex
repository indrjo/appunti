...
\author{Osea}
\date{Primo semestre, A.A. 2024\-2025}
\begin{document}
\maketitle
...
\end{proof}
\begin{corollary}[Beppo Levi per le serie]
...
\begin{proof}[Dimostrazione per \(\varphi , \mu\) misure finite]
    ...
    (E_{0})\), per definizione di derivata di Radon-Nikodym attraverso l'insieme \(\mathcal{F}\) e relative estensioni possiamo assumere che \(\int_{E_{0}} \frac{\partial \varphi }{\partial
...
\(u_{m}\to f\) q.o.~in \(\Omega\) [L:NMT]. Inoltre
\[
...
\end{corollary}
\begin{proof}
    [L:correggere la notazione, g] Se \(f_{n}\to f\) in \(L^{p}{(\Omega)}\) allora è di Cauchy in \(L^{p}\) e
...
anche il valore \(1\). [L: ma nel corollario si parla di un'estratta. Rivedere enunciati] 
...
\begin{enumerate}[label = \arabic*.]
    ...
    \item Se \(\mu{(\Omega)} < +\infty\) e \(1 \le p < q \le \infty\) allora
        \(L^{q}{(\Omega)} \subseteq L^{p}{(\Omega)}\) e si ha che esiste una costante \(C\) per cui
        \[
...
\begin{remark}
    ...
    \]
\end{remark}
\begin{theorem}[Diseguaglianza di Schwarz]
...
\end{example}
\begin{proposition}[Regola del Parallelogramma]
...
\begin{proof}
    ...
    \(f^{+}\) che è finito [L: a che serve che siano maggiorati, mi basta che f^+ - s_n è positivo]. Altrimenti si può anche usare il teorema di
...
\end{example}
\begin{example}
    \(i : \ell^{p} \xhookrightarrow \ell^{q} \) è lineare e limitato con norma
...
Ovviamente invece \(c_{00} \) non è denso in \(\ell^{\infty}\), è allora chiuso?
...
Quindi \(c_{0}\) è \textbf{chiuso} in \(\ell^{\infty}\). 
...
\]
che avrebbe come norma associata la norma \(|\| \cdot \| |_{W^{1,2}{(a,b)}}\).
\begin{proposition}
...
\begin{theorem}[Completezza di \(W^{1,p}(a,b)\)]\label{th:w1p-banach}
    ...
    è uno spazio di Hilbert, con la norma \(\seminorm{\cdot }_{W^{1,2}{(a,b)}} \) 
\end{theorem}
\begin{proof}
    ...
    \end{align*}
    per \(n,m \to \infty\) dove per la terza si è usata la proposizione
    precedente. Ne consegue che \(f_{n}\) e \(f'_n\) sono di Cauchy in
