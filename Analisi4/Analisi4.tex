...
\begin{definition}[label=def:rad_nik_2]{Derivata di Radon-Nykodim con \(\mu\)
    \(\sigma\)-finita}
...
\begin{theorem}[Teorema di Radon-Nykodym più bello]
    Sia \((\Omega, \mathcal{M})\) uno spazio misurabile. Siano \(\mu\) e
    \(\varphi\) rispettivamente una misura \(\sigma\)-finita e una misura
...
\section{TODO} % TODO recuperare
Sia 
\[
    f_{n}{(x)} = {\left( \frac{3}{2} \right)} ^{n} \int_{0} ^{x}\rchi_n{(t)}dt
\]
dove \(\rchi_n\) è la funzione caratteristica di \(C_{n}\), dove ricordiamo che
\(C = \bigcap_{n=1}^{\infty} C_{n} \) è l'insieme di Cantor
\begin{figure}[ht]
    \centering
...
\[
    |\|f\||_{W^{1,p}{(a,b)}} = {\left( \int_a^{b}|f{(x)}|^{p}dx +
...
\begin{eser}
    ...
    \|_{W^{1,p}{(a,b)}} \) e \(|\| \cdot \| |_{W^{1,p}{(a,b)}}\) sono
    equivalenti.
\end{eser}
% TODO fare in modo che la norma tripla sia facile da fare e sia bella da vedere
...
\begin{theorem}[Completezza di \(W^{1,p}(a,b)\)]\label{th:w1p-banach}
    ...
    è uno spazio di Hilbert, con la norma \(|\|\cdot \||_{W^{1,2}{(a,b)}} \) 
\end{theorem}
\begin{proof}
    ...
    % TODO finire la dimostrazione così che è finita
