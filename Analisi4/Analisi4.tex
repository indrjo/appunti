...
\begin{proof}
    ...
    \begin{align*}
        ...
        \sum_{k=1}^{n} c_{k} \Span{e_{k}, x} + \sum_{k,j=1}^{n}
        c_{k}\overline{c_{j}}\Span{e_{k}, e_{j}} = \\ = \|x\|^2 - 2
        \sum_{k=1}^{n}\Re \left(\overline{c_{k}}\Span{x, e_{k}}\right) +
        \sum_{k=1}^{n} + \sum_{k=1}^{n} |\Span{x, e_{k}} |^2 - \sum_{k=1}^{n}
        |c_{k}|^2 |\Span{x, e_{k}} |^2 = \\ = \|x\|^2 - \sum_{k=1}^{n} |\Span{x, e_{k}}
        |^2 + \sum_{k=1}^{n} {\left( |\Span{x, k}|^2 - 2 \Re{\left(
        \overline{c_{k}}\Span{x, c_{k}} + |c_{k}|^2 \right)}  \right)} = \\
        \overset{\eqref{eq:bessel1}}{=}
        \left\|x - \sum_{k=1}^{n} \Span{x, e_{k}}  \right\|^2 + \sum_{k=1}^{n}
        \underbrace{|\Span{x, e_{k}} - c_{k}|^2}_{> 0} > \left\| x - \sum_{k=1}^{n} \Span{x, e_{k}}
        e_{k}\right\|^2
    \end{align*}
...
\begin{theorem}[Caratt.~convergenza della serie di Fourier]\label{thm:conv_Fourier}
...
\end{proposition}
\begin{proof}
Ricordando la definizione~\def{def:separabile} di spazio separabile, ossia
