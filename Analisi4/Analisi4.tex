...
\end{enumerate}
\begin{theorem}[Teorema di generazione di misure]
...
\begin{proposition}[Generazione di misure relative]
    Sia \((\Omega, \mathcal{M}, \mu)\) uno spazio di misura e sia \(f : \Omega
...
\begin{theorem}[Teorema di Radon-Nikodym meno bello]
    Sia \({(\Omega, \mathcal{M})}\) uno spazio misurabile. Siano \(\mu\) e
...
\begin{theorem}[Teorema di Radon-Nikodym più bello]
    Sia \((\Omega, \mathcal{M})\) uno spazio misurabile. Siano \(\mu\) e
    \(\varphi\) rispettivamente una misura \(\sigma\)-finita e una misura
...
\begin{theorem}[Teorema di Lebesgue]
...
\begin{proposition}[Minkovsky o disuguaglianza triangolare]
...
\(c_{0}\) e \(c\) sono invece sottospazi di \(\ell^{\infty}\) 
...
\begin{proof}
    ...
    \dots, 0, \dots)}\) che è la successione canonica. A partire da queste
    consideriamo \(u_k = \varphi {(e^{k})}\) per ogni \(k \in \mathbb{N}\) e
    consideriamo la successione \(u = {(u_k)}\). Vorremmo ora provare che \(u
    \in \ell^{p'}\) e \(\|u\|_{p'} \le \|\varphi \|_{{(\ell^{p})}'} \). A tal
    scopo cominciamo da \(p =1\).
    \[
    ...
    \begin{equation}\label{help:thmduali}
      ...
      \|\varphi \|_{(\ell^{p})'} \|x\|_p
    \end{equation}
    Scegliamo ora \(x_{i} = |u_{i}|^{p'-2}\) e \(x_{i} = 0\) se \(u_{i} =
    0\). Allora nella~\eqref{help:thmduali} abbiamo
    \begin{align*}
        \sum_{i=1}^{k} |u_{i}|^{p'} &\le \|\varphi \|_{(\ell^{p})'} 
            {\left(  \sum_{i=1}^{k} {\left(  |u_{i}|^{p'-1} \right)}^{p}
            \right)}^{\frac{1}{p}} = {\left( \sum_{i=1}^{k} |u_{i}|^{p'p -
            p = p'}  \right)} ^{\frac{1}{p}} \\
            {\left( \sum_{i=1}^{k} |u_{i}|^{p'}  \right)}^{1 - \frac{1}{p}} &=
            {\left( \sum_{i=1}^{k} |u_{i}|^{p'}  \right)} ^{\frac{1}{p'}} \le
            \|\varphi \|_{{(\ell^{p})}'} 
    \end{align*}
    ...
    Usando la densità di \(c_{00} \) in \(\ell^{p}\), quindi per ogni \(x \in
...
\subsection{Funzioni a variazione limitata}
...
La classe di funzioni che invece vogliamo noi è quella delle funzioni
...
Vi sono almeno due problemi di interesse: la serie di Fourier converge? Se
...
\end{remark}
\begin{remark}
    In ambito complesso si può anche prendere (oltre alle funzioni sopra, che
    pure funzionano), le funzioni \(\frac{1}{\sqrt{2\pi}}e^{int}\), con \(n \in
    \mathbb{Z}\) che pure formano una base ortonormale.
\end{remark}
