...
In questa sezione si segue la linea dell'appendice sull'integrazione astratta di Sbordone al libro "Analisi Funzionale" di Brezis, edito da Liguori.
...
L'ipotesi che l'unione della successione d'insiemi appartenga all'algebra risulta pesante, il che motiva la seguente definizione: 
...
naturalmente \(\mu^\star(I) \le |I|\) perché il ricoprimento \(\{I, \emptyset, \dots\} \) fa parte
...
\end{definition}
\begin{remark}
    Con i seguenti risultati si dimostrerà che la misura esterna \(\mu^\star\), ristretta alla classe degli insiemi misurabili (secondo Lebesgue), è una misura.
\end{remark}
\begin{theorem}
    La famiglia \(\mathcal{M}\) degli insiemi misurabili secondo Lebesgue è un'algebra.
\end{theorem}
\begin{proof}
\begin{enumerate}[label = \arabic*.]
...
\begin{itemize}
    ...
    \item[\(\impliedby \)] Per ~\ref{prop:prop_1} e la definizione di \(\inf\) esiste una successione \(I_{n}\) di
        intervalli aperti di diametro \(\le  1\) tali che \(A \subseteq
        ...
        La prima uguaglianza per ~\ref{prop:prop_4} e l'ultima disuguaglianza per l'ipotesi.
\end{itemize}
\end{proof}

\begin{theorem}
    La famiglia \(\mathcal{M}\) degli insiemi misurabili secondo Lebesgue è una
    \(\sigma\)-algebra e \(\mu^\star\) è \(\sigma\)-additiva su
...
\begin{enumerate}[label = \arabic*.]
    \item Sia \(\{A_{n}\} \subseteq \mathcal{M} \) con \(A_{n} \cap A_{m} =
        ...
        Allora \(A = \bigcup_{n \in \mathbb{N}} A_{n} \subseteq I \). Per la proposizione~\ref{prop:prop_5}, dato
        ...
        \mu^\star(A_{i}) > \mu^\star(A) - \frac{\varepsilon}{2} \). Sia \(F := \bigcup_{n = 1}^{k} F_{n} \), che è chiuso. Allora
        ...
        con la proprietà richiesta in ~\ref{prop:prop_5}, sfruttando la limitatezza di \(A\) se ne conclude la misurabilità.
    ...
    \item  Sia \(\{A_{n}\} \subseteq \mathcal{M} \) con \(A_{n} \cap A_{m} =
        ...
        Reticoliamo \(\mathbb{R}^{N}\) con intervalli \(\{I_{j}\} \) \textbf{di diametro fissato} (e.g. 1) tali che
     ...
     e \( j\) intero, esistono \(F_{j}\) chiuso e \(G_{j}\) aperto
        ...
        \]
        Avendo invertito le serie nell'ultimo passaggio (serie assolutamente convergente). Analizziamo la ridotta
        \[
                \begin{aligned}
        ...
        proposizione~\ref{prop:prop_2}. Assieme alla disuguaglianza precedente, per \(n\to
        \infty\) e per arbitrarietà di \(\varepsilon\)  otteniamo che
    ...
    \item Sia \(\{A_{n}\} \subseteq   \mathcal{M}\) e \(A = \bigcup_{i \in
        \mathbb{N}} A_{i} \). Vogliamo provare che \(A \in \mathcal{M}\). La dimostrazione è simile ad alcune già viste.
...
\begin{proof}
    ...
    Gli aperti sono misurabili, infatti un aperto è limitato si può scrivere come unione
    numerabile di intervalli (per la separabilità). Nel caso in cui \(A\) è un aperto qualunque, \(A =
    ...
    \mu^\star(F_{n}) < \frac{1}{n}\) per ogni \(n\) quindi \(\mu^\star(N) = 0\). In
...
Un concetto molto importante della teoria della misura è quello di insieme trascurabile, ovvero di misura nulla.
\begin{example}
    In \(\mathbb{R}\) i punti sono trascurabili e le unioni numerabili di trascurabili sono trascurabili per \(\sigma\)-additività.
...
\[
    ...
    \end{cases}
\]
Allora \(d = 0\) quasi ovunque in \(\mathbb{R}\) (quasi ovunque in \(X\) significa in \(X \sminus N\) dove \(N\) è un trascurabile).
...
\begin{example}[Insieme di Cantor]\label{ex:cantor}
    L'insieme di Cantor è un trascurabile in \(\mathbb{R}\) che ha la cardinalità del continuo.
...
\end{figure}
    ...
    0.2_3\) hanno come immagine \(\frac{1}{2} = 0.1_2 = 0.0\overline{1}_2\). Per determinare che la cardinalità è al più quella del continuo ho bisogno di una funzione iniettiva, ed è sufficiente l'inclusione. Concludo, per il teorema di Schröder–Bernstein, che esiste una biiezione tra \(C\) e \( [0,1]\), ossia \(C\) ha la cardinalità del continuo. 
\end{example}
...
controimmagini di tutte le semirette sono misurabili. Ma quindi anche
tutti i segmenti sono misurabili, siccome \([a, b] = [a, +\infty]
\cap [-\infty, b]\). Essendo misurabili le
controimmagini degli intervalli, abbiamo che anche le
controimmagini dei boreliani sono misurabili (i boreliani sono unioni numerabili di intervalli). 

Le funzioni tipicamente incontrate sono misurabili, costruire una funzione non misurabile non è banale e richiede l'assioma della scelta. 
Ora procediamo con alcune proprietà della classe delle
funzioni misurabili.
\begin{proposition}
...
\end{corollary}
\begin{proof}
    Sia \(g(x) = \inf_n f_{n}(x)\), allora
    \[
        \{x \in \Omega : g(x) \geq \alpha\} = \bigcup_{n=1}^{\infty} \{x \in \Omega
        : f_{n}(x) > \alpha\}
    \]
    poiché significa che esiste almeno un \(n\) tale che \(f_{n}(x) > \alpha\). 

    Sia \(h(x) = \sup_{n} f_{n}(x)\), consideriamo
    \[
        \{x \in \Omega : h(x) > \alpha\} = \bigcap_{n=1}^{\infty} \{x \in
        \Omega : f_{n}(x) > \alpha\}
    \]
...
È chiaro che se \(f\) è misurabile allora ogni \(s_{n}\) è misurabile, gli insiemi scelti sono controimmagini di segmenti (gli \(E_{n,i}\))e una semiretta (\(F_n\)).

Controlliamo che la successione \(s_{n}\) sia monotona non decrescente. Passando da \(s_{n}\) a \(s_{n+1}\) abbiamo un intervallo in
più \([n, n+1)\), quindi se \(f(x) < n\) allora \(s_{n+1}(x) = s_{n}(x)\)
oppure \(s_{n+1}(x) = s_{n}(x) + \frac{1}{2^{n+1}}\). Quindi la
successione è non decrescente.

Consideriamo ora \(f\) di segno qualsiasi. Abbiamo \(f = f^{+} - f^{-}\), dove \(f^{+}\) e \(f^{-}\) sono non negative. Costruiamo due successioni
di funzioni semplici \(s_{n}^{+}\) e \(s_{n}^{-}\) che convergono
rispettivamente a \(f^{+}\) e \(f^{-}\) e osserviamo che \(s_{n} = s_{n}^{+} -
...
\begin{lemma}\label{le:nulle_fuori_compatti}
...
che \(f\) è \textbf{integrabile} (o sommabile) su \(E\) e scrivo \(f \in L^{1}(E)\). 
\end{definition}
...
\begin{theorem}[Teorema di generazione di misure]\label{thm:gen_mis}
    ...
    \rightarrow [0, +\infty]\) una funzione misurabile e non negativa. Allora la funzione
    di insieme 
    \[
    ...
    \begin{enumerate}[label = \arabic*.]
        ...
        \item Siano \(A = \bigcup_{n=1}^{\infty} A_{n} \) con \(A_{n} \in
            ...
            \begin{enumerate}
                ...
                \item se una delle serie corrispondenti a un \(c_{i} >0\) dovesse divergere allora necessariamente l'integrale
            ...
            \]
            per arbitrarietà di \(\varepsilon\) e per \(m \to +\infty\) otteniamo la diseguaglianza desiderata.
    \end{enumerate}
\end{proof}
...
\begin{theorem}[CNES per l'integrabilità]\label{th:cnes_L1}
    ...
    se e solo se esiste \(g\) integrabile tale che \(|f| \le g\) quasi ovunque, in simboli: 
    \[
...
\begin{proof}
    ...
    \[
        \int_E f d\mu \ge \int_E f_{n} d\mu \quad \forall n
    \]
    e quindi, passando al limite per \(n \rightarrow \infty\), 
    \[
...
Nel lemma di Fatou~\ref{le:fatou_base} vale la disuguaglianza larga. L'ugaglianza vale, ad esempio, quando
\(f_{n}\) è crescente in virtù di Beppo Levi~\ref{th:beppo_levi_base}, e
i \(\liminf \)  sono dei \(\lim\). Ci sono casi in cui vale l'ugaglianza stretta. 
...
\end{example}
\begin{example}[Disguaglianza stretta]\label{ex:limite_integrale_NO}
...
\begin{example}[Serie come integrali con misura del contare]
    ...
    misurabili perché tutti gli insiemi sono misurabili. Quali sono le funzioni integrabili? Sia \(f \ge 0\).
    Abbiamo che
    \[
...
\begin{definition}{Convergenza quasi uniforme}
    ...
    che \(\mu(F) < \varepsilon\) e \(f_{n} \rightrightarrows f\) uniformemente in \(\Omega
    \sminus F\).
\end{definition}
...
\begin{example}
    ...
    \mu{(\{1\} )} = 0\). Ma anche \(f_{n} \rightrightarrows f\) quasi uniformemente, infatti
    per ogni \(\varepsilon > 0\) scegliamo \(F=[1-\frac{\varepsilon}{2} , 1]\) e
    nel complementare c'è convergenza uniforme. Infine, per la convergenza in misura, 
    \[
...
\end{example}
Dal precedente esempio possiamo dedurre che q.o.~\(\centernot\implies\) q.u.~e che
q.o.~\(\centernot\implies\) in misura. Ora invece mostriamo le implicazioni che
valgono. 
...
\begin{theorem}[Severini-Egorov]\label{thm:severini-egorov}
    ...
    \(f_{n} \rightrightarrows f\) q.u.~in \(\Omega\).
\end{theorem}
\begin{proof}
...
Vogliamo analizzare gli integrali multipli, per farlo consideriamo gli spazi prodotto. L'insieme ambiente 
...
L'insieme ambiente è naturalmente \(X \times Y\) e definiamo \(\mathcal{M} \times
...
definizione dall misura prodotto: 
...
piccola famiglia monotona di insiemi contenente \(\mathcal{F}\) (attenzione al bisticcio di simboli, qui \(\nu\)
non c'entra nulla con la misura introdotta poc'anzi). 
\begin{remark}
    Dato \(\mathcal{F}\), si possono definire \(\sigma(\mathcal{F})\) e
    \(\nu(\mathcal{F})\), con \(\sigma(\mathcal{F})\) che rappresenta la minima~\(\sigma-\)algebra contenente \(\mathcal{F}\).
\end{remark}
...
\begin{proof}
    ...
    \[
        \begin{aligned}
            ...
            &= \sum_{i=1}^{n} c_{i} \int_X \left( \int_Y \rchi(x, y) \, d\nu \right) d\mu \\
    ...
    \[
        \int_X \left( \int_Y F_{n}(x,y) d\nu \right) d\mu \rightarrow \int_X \left(
        \int_Y F(x,y) d\nu \right) d\mu
    \]
    e quindi abbiamo dimostrato il teorema di Tonelli per funzioni non negative.
\end{proof}
\begin{remark}
    La non-negatività di \(F\) è essenziale per il teorema di Tonelli. Si consideri come esempio
    \[
    f(x, y) =
    \begin{cases} 
...
In questo paragrafo si introduce il concetto di misura relativa, si dimostra il teorema di decomposizione di Hahn 
e si introducono i concetti di misura assolutamente continua e di misura singolare.
\begin{definition}{Misura Relativa}
Sia \((\Omega, \mathcal{M})\) uno spazio misurabile. \(\phi: \mathcal{M} \to
...
\begin{proof}
    ...
    \]
    che è assurdo poiché in uno dei passi successivi \(E\) dovrebbe venir "eliminato" (un certo 
    termine non si può ripetere all'infinito perché la serie \(\sum_{i=1}^{\infty} \frac{1}{n_{i}}\) converge).
\end{proof}
\begin{lemma}
...
\begin{proof}[Dimostrazione del teorema di decomposizione di Hahn]
    ...
    assurdo che non lo sia, ossia che ammetta un certo sottinsieme di misura positiva. 
    Per il primo lemma questo ammete un sottinsieme positivo, e quindi esiste un insieme positivo \(P \subseteq B \) con \(P
...
\begin{proof}
    ...
    0\). Allora, per un risultato precedente, \(|\varphi (E)| \le |\varphi |(E) = 0\), quindi \(\varphi (E) =
    0\) 
\end{proof}
...
\begin{proof}
    ...
    \]
    dove l'ultima uguaglianza è giustificata dal fatto che la serie geometrica converge. 
...
\begin{proof}
    ...
    \begin{itemize}[label = --]
        \item  \(\mu (E \cap A) \le \mu(A) \stackrel{\text{def di } A}{=} 0 \stackrel{\mu-\text{a.c.}}{\implies} \phi(E \cap A) = 0\); 
        \item \(|\phi (E \cap B)| \le |\phi|(E \cap B) \le |\phi| (B) \stackrel{\text{def di } B}{=} 0 \implies \phi(E \cap B)=0\).
    \end{itemize}
    Pertanto, si conclude che \(\varphi(E) = 0\), che completa la dimostrazione.
\end{proof}
\begin{example}
...
\begin{proof}
    ...
    decomposizione che rende \(\varphi \) \(\mu\)-singolare. Intanto è una partizione, infatti 
...
Per dare un'intuizione, sia \(\Omega=\mathbb R\) coi misurabili secondo Lebesgue e la misura \(\mu\) di Lebesgue. Se si ricorda il teorema di generazione di misure \ref{thm:gen_mis} e la relativa generalizzazione per misure relative \ref{prop:gen_mis_rel}, una funzione \(f\in L^1\) genera una misura relativa \(\varphi\): 
\[
\varphi (E) = \int_E f \, d\mu
\]
Nel caso in cui venisse assegnata \(\varphi\) si può invertire il procediemento e trovare \(f\) che la genera? Verrà chimata derivata di Radon-Nikodym quella funzione \(\frac{\partial \varphi}{\partial \mu}\) tale per cui
\[
\int_E \frac{\partial \varphi}{\partial \mu} \, d\mu = \varphi(E)
\]
Si dimostrerà che, in generale, il giusto ambiente per definire una derivata di Radon-Nikodym che abbia questa proprietà è il caso in cui la misura \(\mu\) sia \(\sigma-\)finita e \(\varphi\) sia \(\mu-\)assolutamente continua \ref{thm:rad_nik_ac}.

Inoltre verrà dimostrato che per \(\varphi\) misura relativa \(\mu-\)singolare, ossia per la quale è possibile trovare un insieme \(\mu-\)trascurabile su cui questa misura compie l'interezza della sua variazione, la derivata di Radon-Nikodym è nulla \(\mu-\)quasi ovunque (\ref{thm:rad_nik_singolari}). Ci si può raffigurare il risultato nel caso in cui \(\{x_i\}_{i \in \mathbb N}\) sia una successione di valori reali (quindi trascurabile) e \(\{\alpha_i\}\) dei coefficienti, per cui
\[
...
\]
dove la \(\delta\) è la funzione delta di Dirac. Questa funzione è chiramente nulla \(\mu-\)quasi ovunque.
...
\begin{theorem}[Decomposizione di Lebesgue-Radon-Nikodym]\label{thm:dec_rad_nik}
    ...
    \]
\end{theorem}
\begin{definition}[Derivata di Radon-Nikodym per misure
    finite]\label{def:rad_nik_1}
...
\begin{proof}[Dimostrazione del Teorema~\ref{thm:dec_rad_nik}, ossia
    ...
    \]
    ma la \(f_{2}\) avrebbe dovuto realizzare il \(\sup\), assurdo. Analogo per l'ipotesi assurda che \(\mu(E_2) >0\). Ne consegue
...
\end{enumerate}
    ...
    vero perché \(f_{n} \ge 0 \overset{\text{monotonia}}{\implies} f \ge 0\); \(f \in L^{1}\) per Beppo Levi e
...
\begin{proof}[Dimostrazione per \(\varphi , \mu\) misure finite]
    ...
    \[
        ...
        \end{aligned}
    \]
    dove la disugaglianza si ha perché \(\varepsilon > 0, \mu(A) > 0\). Ma è assurdo perché sappiamo per definizione di \(\frac{\partial \varphi
...
finita] Sia \({\{\Omega_{n}\}}_{n \in \mathbb{N}}\) una partizione di
    ...
    \[
        \begin{aligned}
            \int_E \frac{\partial \varphi}{\partial \mu} \, d\mu &= \int_E \left( \sum_{n=1}^{\infty} \frac{\partial \varphi_n}{\partial \mu_n} \rchi_{\Omega_n} \right) d\mu \\
            &= \sum_{n=1}^{\infty} \int_{E \cap \Omega_n} \frac{\partial \varphi_n}{\partial \mu_n} \, d\mu \\
            &\underset{\text{finite}}{\overset{\text{misure}}{=}} \sum_{n=1}^{\infty} \varphi_n(E \cap \Omega_n) \\
            &= \varphi(E).
        \end{aligned}
    \]
...
e quindi, per definizione di \(\| \cdot \|_1\), abbiamo \(N\) successioni \({\{\alpha_{i,k}\}}_{k \in \mathbb{N}}\)
    ...
    \(\|\cdot \|\) per la disugaglianza 
    \[\|\cdot \| \le M \|\cdot \|_1\] 
    dimostrata nella prima parte. Abbiamo che, per \(\|u_k \| < \frac{1}{k}\) e per convergeza in \(\| \cdot \|\): 
    \[
...
\begin{example}[\(W \subseteq V \), \(W\) Banach, \(V\) no]
    ...
    \textit{In realtà la completezza non è una proprietà topologica, bisognerebbe giustificare meglio, 
...
\begin{itemize}[label = --]
    ...
    \item \emph{Diseguaglianza triangolare} meno banale delle precedenti, dimostrata più avanti sotto il nome di 
...
\begin{proposition}[Minkowski o disuguaglianza triangolare]
...
\begin{proof}
    ...
    \begin{align*}
        \int_\Omega |f+g|^{p} d\mu &= \int_\Omega |f+g|^{p-1} |f+g| d\mu \\
                                    &\le \int_\Omega \underbrace{|f+g|^{p-1}}_{:= w} |f| d\mu + \int_\Omega |f+g|^{p-1} |g| d\mu
    \end{align*}
    ...
    triangolare
\end{proof}
Con la precedente proposizione abbiamo concluso la dimostrazione di~\ref{prp:spazio-Lp-normato}, ossia che \(L^{p}\) è uno spazio
normato.
...
\begin{proof}[Dimostrazione per \(p=+\infty\)]
    ...
    Allora, per~\eqref{eq:helper_compllp}, si ha \(|f_{n}{(x)} - f_{m}{(x)}| \le \frac{1}{k}\) per ogni \(x \in
    \Omega \sminus N\).
...
\begin{proof}[Dimostrazione con \(1 \le p < +\infty\)]
    ...
    \begin{aligned}
        ...
        n_2 &:= \max\{n_{\varepsilon = \frac{1}{2^2}}, n_1 + 1\} &&\leadsto f_{n_2}, \\
        &\vdots \\
        n_k &:= \max\{n_{\varepsilon = \frac{1}{2^k}}, n_{k-1} + 1\} &&\leadsto f_{n_k}.
    \end{aligned}
    \]
    e abbiamo che
    \[
    ...
    Definiamo ora 
    \[u_k := f_{n_k} \] 
    vogliamo mostrare che \(u_k\) converge in \(L^{p}\) a qualche elemento \(f\). A tale scopo sia
    \[
        g_k(x) := \sum_{i=1}^{k} \left| u_{i+1} {(x)} - u_{i}{(x)} \right|  
    \]
    che è la ridotta di una serie a termini non negativi e ogni \(g_k
    ...
    \[
        ...
        \le 1
    \]
    Allora \(h_k := |g_k|^{p}\) è una successione monotona crescente a termini non negativi. 
...
\begin{proof}
    ...
    quindi, come si può notare nella dimostrazione del teorema, esiste un'estratta \(u_k := f_{n_k} \) tale che \(u_k\)
    converge q.o.~a una funzione \(\tilde{f}\) e inoltre, per \(g\) definito come nella dimostrazione del teorema,
\[
...
Si dimostrano (3) e (4).
\begin{itemize}
    \item [(3)] Sia \(f_n \rightarrow f\) in \(L^p\). Definiamo
        \[A_n := \{x \in \Omega: \quad |f_n(x) - f(x)| > \varepsilon\}\]
        e quindi abbiamo
        \[
            \begin{aligned}
                0 &\le \varepsilon^p \mu(A_n) \\
    ...
    \item [(4)] Sia \(f \in L^{q}{(\Omega)}\). Allora, usiamo la
    diseguaglianza di Hölder con esponenti \(\frac{q}{p}\) e \(\frac{q}{q-p}\) (che chiaramente sono coniugati):
    \[
...
\end{example}
\begin{example}
    L'altra inclusione è normalmente falsa. Ad esempio se \(f{(x)} =
...
\begin{enumerate}[label = \arabic*.]
    ...
    \item Per ogni \(\delta > 0\) esiste una funzione \(u \in
        ...
        \begin{align*}
            ...
            &= \int_{-\overline{n} \le u \le \overline{n}} |u - h_{\overline{n}}| \, d\mu 
                + \int_{u > \overline{n}} (\overline{n} - h_{\overline{n}}) \, d\mu 
                + \int_{u < -\overline{n}} (h_{\overline{n}} + \overline{n}) \, d\mu \\
            &\le \left( \int_{-\overline{n} \le u \le \overline{n}} 
                + \int_{u > \overline{n}} 
                + \int_{u < -\overline{n}} \right) |u - h_{\overline{n}}| \, d\mu.
        \end{align*}
        
    \end{enumerate}
    Osservo che \(v - h_{n} \in L^1(\mathbb{R}^{N}) \cap
    L^{\infty}{(\mathbb{R}^{N})}\) e inoltre \(|v - h_{\overline{n}}| \le
    2\overline{n}\). Allora, per \ref{thm:Holder}: 
    \begin{align*}
...
\begin{example}[Operatore lineare non continuo]
    ...
    Questa successione converge unifiormemente a \(0\) poiché il modulo è limitato da \(1/n\) sul dominio.
    Tuttavia 
    \[T(f_n) = \frac{n^2 \cos (n^2 \cdot 0)}{n} = n \rightarrow \infty\]
    Ossia 
    \[T(f_n) \centernot{\rightarrow} T(0) = 0\]
...
\begin{example}
    ...
    \frac{1}{q}}\). Vogliamo capire se c'è l'ugaglianza o meno. Per l'ugaglianza dobbiamo
    trovare un elemento \(0 \not\equiv f \in L^{p}{(\Omega)}\).
    Sia \(f = 1\), allora
    \begin{align*}
...
\end{proposition}
\begin{proof}
    Si tratta di una riproposizione dell'argomento diagonale di Cantor.
    ...
    \[
      ...
      \end{cases}
    \]
    che appartiene a \(\ell^{\infty}\) in quanto è limitata da \(2\), ma è staccata dalle altre successioni considerate:
    \[\|z - x^{k}\|_{\infty} = \sup_n |z_{n} - x^{k}_n| \ge |z_k - x^{k}_k| \ge
    1\] 
    Si ha una contraddizione e quindi la successione \(x= {(x^{k})}\)
    non è densa in \(\ell^\infty\), segue la non separabilità di \(\ell^\infty\). 
\end{proof}
...
\begin{proof}
    ...
    \begin{aligned}
        \sum_{i=1}^{k} |u_{i}|^{p'} 
        &\le \|\varphi\|_{(\ell^{p})'} 
        \left( \sum_{i=1}^{k} \left( |u_{i}|^{p'-1} \right)^{p} \right)^{\frac{1}{p}} \\
        &= \|\varphi\|_{(\ell^{p})'} \left( \sum_{i=1}^{k} |u_{i}|^{p'p - p} \right)^{\frac{1}{p}} \\
        &= \|\varphi\|_{(\ell^{p})'} \left( \sum_{i=1}^{k} |u_{i}|^{p'} \right)^{\frac{1}{p}}
    \end{aligned} 
    
    \]
   e quindi 
   \[\left( \sum_{i = 1}^{k} |u_i|^{p'} \right)^{\left( 1-\frac{1}{p} \right)= \frac{1}{p'}} \le \| \phi\|_{(\ell^p)'}\]
...
\begin{definition}{funzione a variazione limitata}
    ...
    finito, cioè 
    \[
        V_f ([a, b]):= \sup \{v_s : S \text{ sudd. di \([a,b]\) in un numero finito di
        punti  } \} \in \mathbb{R} 
    \]
...
\end{remark}
\begin{proof}
Il fatto che la differenza di due funzioni monotone non decrescenti sia una funzione a variazione limitata è banale per la disugaglianza triangolare. Per il viceversa, sia \(f\) a variazione limitata, definiamo
\[
    g_1 : = V_f[a, x]
\]
...
\end{remark}
\begin{proof}
    Si riconduce a dimostrare che una funzione monotona su un intervallo è derivabile quasi ovunque a derivata sommabile. La dimostrazione è leggermente laboriosa e viene qui omessa. 
\end{proof}
...
che stiamo cercando. In realtà no. Ad esempio la funzione di Heaviside (l'indicatrice dei positivi)
considerata in \([-1, 1]\) è a variazione limitata, \(H'\) esiste e vale 0
...
allora la serie \(f_{n}\) converge uniformemente a una funzione limite \(f{(x)}\), definita funzione di Vitali.
...
La classe di funzioni che invece soddisfa le proprietà volute è quella delle funzioni \emph{assolutamente continue}.  
\begin{definition}{Assoluta continuità}
    \(f : [a, b] \to \mathbb{R}\)  si dice assolutamente continua se per ogni
...
variazione limitata. Bisogna solo avere l'accortezza di avere delle sovrapposizioni degli intervallini per evitare la perdita di arbitrarietà delle suddivisioni (che comunque posso restringere sufficientemente senza perdita di generalità). 
\end{proof}
...
\]
chiaramente quindi \(W^{1,1}{(a,b)} = AC{([a,b])}\) e succede che se \(1 \le p < q \le \infty\) allora \(W^{1,q}{(a,b)} \subseteq W^{1,p}{(a,b)} \) per le
...
Sul piano euclideo \(H = \mathbb{R}^2\) è intuitivo il concetto di proiezione~\(P_K : \mathbb R^2 \rightarrow K\) che manda
un punto \(z\in \mathbb R^2\) in un punto di un insieme convesso e chiuso \(K\) che minimizza 
la distanza da \(z\), come in figura ~\ref{fig:teorema-delle-proiezioni}.
\begin{figure}[ht]
    \centering
...
\end{figure}
Nel caso in cui \(z \in K\) la proiezione manda banalmente \(z \mapsto z \quad \forall z \in K\). 
...
\begin{remark}
    Una maniera per memorizzare la disugaglianza variazionale è immaginare il caso di un convesso \(K \subset\mathbb R^2\), in cui se ci si muove da \(f\) alla proiezione \(u\) (quindi lungo il vettore \(u-f\)) e poi da \(u\) a un generico \(v\) in \(K\) (ossia \(v -u\)), l'angolo non orientato che si forma è minore di \(\pi/2\), e quindi il prodotto scalare è positivo, che diventa negativo scambiando il segno del secondo termine.
\end{remark}
...
\end{enumerate}
\begin{proof}[Passo 1: Esistenza della soluzione di~\eqref{eq:PM}]
...
\end{proof}
\begin{proof}[Passo 2: \(u\) sol.~\eqref{eq:PM} \(\implies u \) sol.~\eqref{eq:DV}]
...
\right| &\le \|f_{1} -f_{2}\|\|u_{1} - u_{2}\| 
\end{align*}
con l'ultima disugaglianza dovuta a Schwarz, da cui otteniamo che, se \(u_{1} \neq u_{2}\), \(\|u_{1} - u_{2}\| \le \|f_{1}
...
\begin{proof}
    ...
    \endaligned
    \]
    poiché la disugaglianza vale con il \(\mp\), nel complesso consideriamo anche \(w = u \pm iv\),
...
\begin{note}
    ...
    particolare avevamo trovato che \({(\ell^2)}' = \ell^2\), che potrebbe essere
...
\begin{proof}
    ...
    sottospazio (controimmagine di un operatore lineare) ed è chiuso in \(H\) (controimmagine di
...
\|x\|^2 - \sum_{k=1}^{n} |\Span{x, e_{k}} |^2
    ...
    se \(c \in \ell^2\). Sono tutte equivalenze e quindi si è stabilita l'equivalenza tra le due condizioni. 
\end{proof}
\begin{remark}
...
\]  
In particolare, controlliamo che valga per gli elementi della base \(\{e_j\}_{j=1, \dots, n}\) di \(V_n\):  
\[
\begin{aligned}
...
associata alla \textit{(iv)} per qualche motivo, forse legato all'intuizione 
...
\begin{enumerate}[label = \alph*.]
    ...
    \item Per ogni \(\delta \in {(0, \pi)}\) si ha 
        \[
          \lim_{n \to \infty} {\left( \sup_{\delta \le |t| \le \pi} q_{n}{(t)} \right)} 
        \]
\end{enumerate}
\end{lemma}
\begin{proof}
...
\begin{proof}[Dimostrazione di Weierstrass]
    ...
    \]
    per opportuni coefficienti. La funzione \(f\) è continua in \(\mathbb R\) e periodica di periodo \(2\pi\). Si ricorda la definizione del prodotto di convoluzione: 
    \[
        p_n(t) = (f \star q_n)(t) := \int_{-\pi}^\pi f(t-s)q_n(s) \, ds = \int_{-\pi}^\pi f(s)q_n(t-s) \, ds = 
    \]
    ossia vi è commutatività (si mostra con un cambio di variabile e ricordando la periodicità di \(f\)). In generale si mostra che il prodotto di convoluzione tra una fuzione in \(L^2(-\pi, \pi)\) e un polinomio trigonometrico è anch'esso un polinomio trigonometrico, ad esempio con l'identità di Eulero. Nel nostro caso particolare si vede che 
    \[
\begin{aligned}
    p_n &= \int_{-\pi}^\pi f(s) q_n(t-s) \, ds = \\
        &= \int_{-\pi}^\pi a_0 f(s) \, ds + \\
...
\[
\begin{aligned}
    |f(t) -p_n(t) | &\overset{(b)}{=} \left| f(t) \frac{1}{2\pi} \int_{-\pi}^\pi q_n(s) \, ds -  \frac{1}{2\pi}\int_{-\pi}^\pi f(t-s)q_n(s) \, ds\right| = \\
    &= \frac{1}{2\pi} \left| \int_{-\pi}^\pi (f(t) - f(t-s)) q_n(s) \, ds \right|
\end{aligned}
\]
Usando la continuità su \(\mathbb R\) e la periodicità ci si può ricondurre alla continuità su un compatto e quindi \(f\) è continua uniformemente. Per concludere, fissato \(\varepsilon\) positivo:
\[
\begin{aligned}
    |f(t) - p_n(t)| 
...
\]
Quindi è chiaro che \(p_n \rightrightarrows f\). 
...
\begin{proof}
    Si ricorda che l'insieme delle funzioni continue a supporto compatto \(C_C^0(\pi, \pi)\) è denso in \(L^p(-\pi, \pi)\) per ogni \(p\) intero positivo (si veda \ref{thm:risultato_4}). Nel caso di \(p=2\):
    \[
        \forall \varepsilon >0 \quad \exists g \in C_C^0 (-\pi, \pi): \quad \| f-g\|_{L^2(-\pi, \pi) } \leq \varepsilon
    \]
    Possiamo prolungare \(g\) per periodicità \(2 \pi\) a tutto \(\mathbb R\), e la chiamiamo ancora \(g\) per semplicità. La funzione \(g\), siccome è a supporto compatto in \((-\pi, \pi)\), si attacca bene e quindi abbiamo continuità in \(\mathbb R\). Per il teorema di Weierstrass (\ref{thm:weierstrass}) esiste un polinomio trigonometrico \(p_\varepsilon\): 
    \[
    \| g - p_\varepsilon\| \leq \varepsilon
    \]
    Si ha la seguente disugaglianza: 
   \[
    \begin{aligned}
...
\end{proof}
I seguenti corollari sono semplici calcoli/conseguenze del teorema sulle quattro condizioni equivalenti (\ref{thm:conv_Fourier}). 
\begin{corollary}
...
\begin{proof}
    La funzione \(f \in C^0[a, b]\) può essere prolungata per parità e ridefinita su \([a, 2b-a]\) e poi prolungata per periodicità con periodo di lunghezza \(2(b-a)\). Siamo nelle condizioni del teorema di Weierstrass (\ref{thm:weierstrass}), anche se con un periodo diverso. Sia \(\pi_\varepsilon\) il polinomio tale che 
    \[
    ...
    \]
    Per la disugaglianza triangolare la tesi è dimostrata. 
    
\end{proof}

Si è detto che, presa \(f\) in \(L^2(-\pi, \pi)\), la serie di Fourier relativa converge a \(f\) in \(L^2(-\pi, \pi)\). Per la proposizione \ref{prop:converenze_Lp} la serie ammette un'estratta di ridotte che converge quasi ovunque a \(f\). Per il teorema di \textbf{Carleson} (1966) si può dire di più: c'è convergenza quasi ovunque dell'intera successione delle ridotte. Tale risultato era stato congetturato da Lusin nel 1915. 

Se si aggiunge l'ipotesi di regolarità a tratti si può affermare la convergenza della serie di Fourier a \(f\) eccetto nei punti di discontinuità (di tipo salto), dove \(f\) converge alla media tra il limite destro e quello sinistro (che esistono). 

Se invece si ipotizza \(f\) solo continua e non si introducono ipotesi sulla continuità della derivata, non è detto che la serie di Fourier converga in ogni punto. Un controesempio è dovuto a \textbf{du Bois-Reymond} (1873), che costruise una funzione continua la cui serie di Fourier diverge in un insieme denso. Tale risultato viene poi rafforzato con un controesempio di \textbf{Kolmogorov} (1923) che trova una funzione \(L^1\) la cui serie di Fourier diverge quasi ovunque, e nel 1926 ovunque. 

Per quel che riguarda la convergenza uniforme si possono trovare delle condizioni sufficienti sui coefficienti di \(f\). Una condizione, per il criterio di Weierstrass (sulla convergenza totale, detto M-test) è la convergenza della serie \(\sum_{k=1}^\infty |a_k| + |b_k|\). Si possono anche trovare condizioni più interessanti, come ad esempio \(f_{|[-\pi, \pi]} \in W^{1,2}(-\pi, \pi)\), ricordando la notazione degli spazi di Sobolev. La dimostrazione si riconduce alla convergenza totale poc'anzi citata: 

\begin{proposition}[Condizione sufficiente convergenza serie di Fourier]
    Se \(f: \mathbb R \rightarrow \mathbb R\) è periodica di periodo \(2\pi\) e \(f_{|[-\pi, \pi]} \in W^{1, 2}(-\pi, \pi)\) allora la serie di Fourier converge uniformemente a \(f\).
\end{proposition}
...
\begin{proof}
    ...
    a_k &=    \frac{1}{\sqrt{\pi}} \int_{(-\pi, \pi)} f(t) \cos(kt) dt  = -\frac{1}{k} \frac{1}{\sqrt{\pi}} \int_{-\pi}^\pi f'(t) \sin(kt) \, dt = -\frac{1}{k}b_k' \\
    b_k &= (\dots) = \frac{1}{k}a_k'
    \end{aligned}     
    \]
    Siccome siamo in \(W^{1,2}(-\pi, \pi)\), abbiamo che \(f' \in L^2(-\pi, \pi)\) e quindi \(\{a_k'\}, \{b_k'\} \in \ell^2\). Per la disuglianza di Young (): 
    \[
    \begin{aligned}
    ...
    \]
    converge uniformemente, e quindi la serie di Fourier converge uniformemente a \(f\).
\end{proof}
...
\begin{enumerate}[label = \alph*.]
    \item Dato \(\varepsilon > 0\), è possibile approssimare \(f\) con una
        ...
        in \(L^2{(-\pi, \pi)}\) e scegliere dunque \(g_\varepsilon \in
        C_C^{0}{(0, 2\pi)}\). 
