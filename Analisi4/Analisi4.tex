...
\begin{theorem}
    Ogni spazio \(L^{p}\) è completo
\end{theorem}
\begin{proof}[dimostrazione per \(p=+\infty\)]
    % TODO
\end{proof}
\begin{proof}[dimostrazinoe con \(1 \le p < +\infty\)]
...
\[
    ...
    \in L^{1}{(\Omega)}
\]
e possiamo applicare il teorema di convergenza dominata di Lebesgue
(Teorema~\ref{thm:Lebesgue_conv_dom}) per cui
\[
...
Il corollario si può espandere a dire che tutta la funzione converge quasi
