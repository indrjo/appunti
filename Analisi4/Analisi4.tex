...
NB:~\emph{Sommabile} è un sinonimo di \emph{integrabile} e capiterà di
usarlo.
...
Ricordando il lemma di Fatou~\ref{le:fatou_base}, avevamo che la diseguaglianza
...
\begin{lemma}[Lemma di Fatou \-- versione estesa]\label{th:fatou_esteso}
    Sia \((\Omega, \mathcal{M}, \mu)\) uno spazio di misura. Sia \(\{f_{n}\}\) una
...
\begin{theorem}[Convergenza Dominata di Legesgue]\label{thm:Lebesgue_conv_dom}
    Sia \({(\Omega, \mathcal{M}, \mu)}\) uno spazio di misura. Sia \(f_{n}\) una
...
\begin{proposition}[Generazione misura relativa]
    Sia \((\Omega, \mathcal{M}, \mu)\) uno spazio di misura e sia \(f : \Omega
...
\begin{theorem}[Teorema di Radon-Nyokym meno bello]
    Sia \({(\Omega, \mathcal{M})}\) uno spazio misurabile. Siano \(\mu\) e
...
\end{proof}
\begin{proof}
\textbf{Inserire dimostrazione di Weierstrass}.
\end{proof}
...
\begin{proof}
    \textbf{Inserire dimostrazione}.
\end{proof}

\begin{corollary}
    Data \(f \in L^2(-\pi, \pi)\), la serie di Fourier associata: 
    \[
    \frac{a_0}{2\pi} + \sum_{k=1}^{\infty} \left( \frac{a_k}{\sqrt{\pi}} \cos(kt) + \frac{b_k}{\sqrt{\pi}} \sin(kt)\right)
    \]
    dove
    \[
    \begin{aligned}
