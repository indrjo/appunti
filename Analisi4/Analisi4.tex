...
\begin{enumerate}[label = \arabic*.]
    \item (monotonia) Se \(A, B \in \mathcal{M}\) e \(A \subseteq B \) allora \(\mu(A) \le
        \mu(B)\) 
    \item (sottratività) Se \(A, B \in \mathcal{M}\) e \(A \subseteq B \) e \(\mu(A) <
...
\begin{proof}
    ...
    prendiamo \[E_{n, i} = \{x \in \Omega : \frac{i-1}{2^{n}} \le f(x) <
    \frac{i}{2^{n}}\} \quad \forall n \in \mathbb{N}, \,\, \forall i \in \{1,
\dots, n2^{n}\}  \] 
...
\begin{definition}[Integrale di Lebesgue]
    ...
    \[
        \int_E f \, d\mu = \sup \{I_{E}(s) : s \text{ semplice }, 0 \le s \le f
        \text{ in \(E\) }\}
    \]
...
\begin{enumerate}[label = \arabic*.]
    ...
    \begin{proof}
        \(f = f^{+} - f^{-}\) e abbiamo che \(a^+ \le f^{+} \le b^{+}\) e
        \(a^{-} \ge f^{-} \ge b^{-}\). Le funzioni che valgono \(a^{+}, b^{+}, a^{-},
        b^{-}\) su \(E\) sono funzioni semplici misurabili con integrale dato da
        \(a^{+} \mu(E), b^{+} \mu(E), a^{-} \mu(E), b^{-} \mu(E)\)
        rispettivamente. %TODO: finire
    \end{proof}
...
\begin{proof}
    \begin{enumerate}[label = \arabic*.]
        ...
        \item Siano \(A = \bigcup_{n=1}^{\infty} A_{n} \) con \(A_{n} \in
            ...
            \(c_{i} >0\) dovesse divergere allora necessariemente l'integrale
...
\begin{corollary}[ \(\sigma\)-additività dell'integrale]
    Se \(f \in L^{1}(\Omega)\) e \(\{A_{n}\}\) è una successione di insiemi
    disgiunti a due a due, con \(A = \bigcup_{n=1}^{\infty} A_{n} \) 
    \[
...
\begin{proof}
    ...
    \]
\end{proof}
\begin{theorem}[CNES per l'integrabilità]
...
\begin{itemize}[label = --]
    ...
    \item se \(\alpha > 0\) dobbiamo studiare il comportamente vicino a \(0\).
        ...
        \]
        procedendo con l'hopital finché l'esponente non diventa negativo
...
\end{example}
\begin{remark}
    Ci sono funzioni integrabili in senso improprio che non L-integrabili.
    \begin{example}
...
\]
Ossia è una famiglia chiusa rispetto all'unione crescente e all'intersezione
decrescente.
\end{definition}
...
\begin{theorem}[Tonelli]
    ...
    \begin{itemize}
        \item \(F(x, y)\) è misurabile sia rispatto a \(x\) per q.o.~\(y\) che
...
\begin{theorem}[Decomposizione di Lebesgue-Radon-Nikodym]
    Sia \((\Omega, \mathcal{M})\) uno spazio misurabile. Siano \(\mu\) e
    \(\phi\) due misure finite su \((\Omega, \mathcal{M})\). 
    Sia ora
    \[
...
\begin{definition}[Derivata di Radon-Nikodym]
    ...
    denotata con
    \[
        f = \frac{\partial \phi}{\partial \mu } \quad ;\quad \int_\Omega \frac{\partial
        \phi}{\partial \mu } d\mu = \sup_{g \in \mathcal{F}} \int_\Omega g d\mu
    \]
\end{definition}
...
\end{proof}
In uno spazio vettoriale \(V\) si possono consdierare più norme, vediamo alcuni
